import React, { useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';
import Button from '../common/Button';

const LoginPageImage = '/images/LoginPage.webp';
const BusBuddyLogo = '/images/BusBuddyLogo.webp';

const GoogleLogin: React.FC = () => {
  const { signInWithGoogle, user, loading, error } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();

  const from = location.state?.from?.pathname || '/';

  useEffect(() => {
    if (user) {
      navigate(from, { replace: true });
    }
  }, [user, navigate, from]);

  const handleGoogleSignIn = async () => {
    await signInWithGoogle();
  };

  return (
    <div className="min-h-screen w-full flex bg-[#0D0A2A] text-white font-sans overflow-hidden">

      <div className="hidden lg:flex lg:w-[65%] relative">
        <img
          src={LoginPageImage}
          alt="Bus Buddy Illustration"
          className="absolute inset-0 w-full h-full object-cover"
        />

        <div className="absolute inset-0 bg-[#0D0A2A]/40 mix-blend-multiply"></div>
        <div className="absolute inset-0 bg-gradient-to-t from-[#0D0A2A] via-transparent to-transparent"></div>

        {/* Branding Overlay on the Image */}
        <div className="relative z-10 p-12 flex flex-col justify-between h-full">
          <div className="flex items-center gap-3">
            <img src={BusBuddyLogo} alt="BusBuddy Logo" className="w-12 h-12 drop-shadow-lg" />
            <h1 className="text-4xl font-bold tracking-tight">
              Bus<span className="text-[#B045FF]">Buddy</span>
            </h1>
          </div>
          <div className="mb-10">
            <h2 className="text-5xl font-bold mb-4 leading-tight">
              Your Campus Commute, <br />
              <span className="text-[#B045FF]">Simplified.</span>
            </h2>
            <p className="text-gray-300 text-xl max-w-lg">
              Real-time tracking, accurate schedules, and a smoother ride to college every day.
            </p>
          </div>
        </div>
      </div>

      {/* 2. RIGHT SIDE: Login Section */}
      <div className="w-full lg:w-[35%] flex flex-col items-center justify-center p-6 relative">
        <div className="absolute lg:hidden top-0 right-0 w-64 h-64 bg-[#B045FF] blur-[100px] opacity-20"></div>

        <div className="absolute top-8 left-8 lg:hidden flex items-center gap-2">
          <img src={BusBuddyLogo} alt="BusBuddy Logo" className="w-8 h-8" />
          <h1 className="text-2xl font-bold">
            Bus<span className="text-[#B045FF]">Buddy</span>
          </h1>
        </div>

        <div className="w-full max-w-sm space-y-8 z-10">
          <div className="text-center">
            <h2 className="text-3xl font-bold tracking-tight">
              Welcome Back
            </h2>
            <p className="mt-2 text-gray-400">
              Sign in to access your dashboard
            </p>
          </div>

          {error && (
            <div className="bg-red-500/10 border border-red-500/50 rounded-lg p-4">
              <p className="text-sm text-red-200 text-center">{error}</p>
            </div>
          )}

          {/* THE GOOGLE BUTTON SECTION */}
          <div className="mt-8">
            <div className="relative mb-8">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-gray-700"></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-[#0D0A2A] text-gray-400">
                  Continue with
                </span>
              </div>
            </div>

            <div className="bg-white rounded-lg hover:bg-gray-50 transition-colors duration-200">
              <Button
                onClick={handleGoogleSignIn}
                isLoading={loading}
                fullWidth
                variant="outline"
                className="flex justify-center items-center gap-3 py-3 border-0 text-gray-900 font-semibold"
              >
                <svg className="h-5 w-5" viewBox="0 0 24 24">
                  <path
                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    fill="#4285F4"
                  />
                  <path
                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    fill="#34A853"
                  />
                  <path
                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    fill="#FBBC05"
                  />
                  <path
                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    fill="#EA4335"
                  />
                </svg>
                <span>Sign in with Google</span>
              </Button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default GoogleLogin;